        ...
        try:
            ...
            if loop and loop.is_running():
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                    execution_record = pool.submit(lambda: asyncio.run(execute_simulated_action(**coro_args))).result()
            else:
                execution_record = asyncio.run(execute_simulated_action(**coro_args))
        except Exception:
            execution_record = {
