...
import { 
  ...
  Lock, Check, Clock, UserCheck, Scale, AlertCircle, FileCheck, HelpCircle, Sparkles
} from 'lucide-react';
...
const DecisionSupportView = ({ data, onChallengeFinding }) => {
          ...
          <div className="space-y-2">
            {steps.map((step, idx) => (
              <div key={step.step_id || idx} className="p-3.5 rounded-xl border border-[#1E293B] bg-[#060B14] space-y-2">
                ...
                {/* Supporting evidence references and challenge action */}
                <div className="pl-7 pt-2 border-t border-[#1E293B]/60 flex flex-wrap items-center justify-between gap-2">
                  {(step.supporting_evidence_ids?.length > 0 || step.supporting_regulatory_ids?.length > 0) ? (
                    <div className="flex flex-wrap gap-1.5 items-center text-[10px] font-mono text-slate-500">
                  ...
                  <button
                    type="button"
                    onClick={() => onChallengeFinding?.({
                      id: step.step_id || `STEP-${idx + 1}`,
                      title: step.action,
                      stage: 'DECISION_SUPPORT',
                      severity: step.priority || 'STANDARD',
                      source: 'Analyst Decision Support Agent',
                      evidenceIds: step.supporting_evidence_ids || [],
                      description: step.rationale || step.action
                    })}
                    className="px-2.5 py-1 rounded-lg bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/30 text-sky-300 font-mono text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ml-auto"
                    title="Challenge or question this recommended step"
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {dispositions.map((disp, idx) => (
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-1 pt-1.5 border-t border-[#1E293B]/60 text-[9px] font-mono text-slate-500">
                  ...
                  <button
                    type="button"
                    onClick={() => onChallengeFinding?.({
                      id: disp.action_code || `DISP-${idx + 1}`,
                      title: disp.label || disp.action_code,
                      stage: 'DECISION_SUPPORT',
                      severity: disp.recommended ? 'HIGH' : 'STANDARD',
                      source: 'Policy & Decision Matrix',
                      evidenceIds: [],
                      description: disp.description
                    })}
                    className="px-2 py-0.5 rounded bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/30 text-sky-300 font-mono text-[9px] font-bold uppercase tracking-wider flex items-center gap-1 transition-all cursor-pointer"
                    title="Challenge or question this disposition option"
                  >
...
const AuditExplanationView = ({ data, onChallengeFinding }) => {
                ...
                <div className="pl-7 pt-2 border-t border-[#1E293B]/60 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    onClick={() => onChallengeFinding?.({
                      ...
                      title: item.title || item.stage || `Narrative Step ${idx + 1}`,
                    ...
                    className="px-2.5 py-1 rounded-lg bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/30 text-teal-300 font-mono text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ml-auto"
                    title="Challenge or question this narrative step"
                  >
              ...
              <div key={kf.finding_id || idx} className="p-3 rounded-xl border border-[#1E293B] bg-[#060B14] space-y-1.5">
                ...
                <div className="pt-2 border-t border-[#1E293B]/60 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => onChallengeFinding?.({
                      id: kf.finding_id || `KF-${idx + 1}`,
                      title: kf.statement,
                      stage: 'AUDIT_EXPLANATION',
                      severity: kf.severity || 'HIGH',
                      source: 'Audit Explanation Agent',
                      evidenceIds: kf.supporting_evidence_ids || [],
                      description: kf.statement
                    })}
                    className="px-2.5 py-1 rounded-lg bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/30 text-teal-300 font-mono text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ml-auto"
                    title="Challenge or question this key finding"
                  >
                    <HelpCircle className="w-3 h-3 text-teal-400" />
                    <span>CHALLENGE AGENT</span>
                  </button>
...
const RegulatoryView = ({ data, onChallengeFinding }) => {
              ...
              <div key={ind.id || idx} className="p-3.5 rounded-xl border border-[#1E293B] bg-[#060B14] space-y-2">
                ...
                <div className="pt-2 border-t border-[#1E293B]/60 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    onClick={() => onChallengeFinding?.({
                      id: ind.id || `REG-${idx + 1}`,
                      title: `${ind.code || ind.indicator_code || 'REG'}: ${ind.description || ind.indicator}`,
                      stage: 'REGULATORY',
                      severity: ind.severity || 'CRITICAL',
                      source: ind.regulatory_framework || 'PMLA / FIU-IND Framework',
                      evidenceIds: ind.supporting_evidence_ids || [],
                      description: ind.description || ind.indicator
                    })}
                    className="px-2.5 py-1 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-300 font-mono text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ml-auto"
                    title="Challenge or question this regulatory indicator"
                  >
...
const ContextualView = ({ data, onChallengeFinding }) => {
              ...
              <div key={p.pattern_id || idx} className="p-3.5 rounded-xl border border-purple-500/20 bg-[#060B14] space-y-2">
                ...
                <div className="pt-2 border-t border-[#1E293B]/60 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => onChallengeFinding?.({
                      id: p.pattern_id || `CP-${idx + 1}`,
                      title: p.name || p.pattern_name || 'Pattern Matching Finding',
                      stage: 'CONTEXTUAL',
                      severity: p.severity || 'HIGH',
                      source: 'Contextual Investigation Engine',
                      evidenceIds: p.supporting_evidence_ids || [],
                      description: p.description || p.reasoning
                    })}
                    className="px-2.5 py-1 rounded-lg bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 text-purple-300 font-mono text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ml-auto"
                    title="Challenge or question this agent finding"
                  >
                ...
                <div className="pt-2 border-t border-[#1E293B]/60 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    onClick={() => onChallengeFinding?.({
                      ...
                      evidenceIds: f.supporting_evidence_ids || [],
...
const EvidenceView = ({ data, onChallengeFinding }) => {
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-2.5 pt-2 border-t border-[#1E293B]/60 flex flex-wrap justify-between items-center gap-2">
                {hasExtraData ? (
                  <button
                    type="button"
                ...
                <button
                  ...
                  onClick={() => onChallengeFinding?.({
                    id: item.id,
                    title: item.finding || item.statement || `${item.indicator} = ${item.value}`,
                    stage: 'EVIDENCE',
                    severity: item.severity,
                    source: item.source || 'Evidence Collector',
                    evidenceIds: [item.id],
                    description: item.finding || item.statement || `${item.indicator} = ${item.value}`
                  })}
                  className="px-2.5 py-1 rounded-lg bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/30 text-sky-300 font-mono text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ml-auto"
                  title="Challenge or question this agent finding"
                >
...
export const AnalystEvidenceViewer = ({ 
  ...
  const [challengeFinding, setChallengeFinding] = useState(null);

  const effectiveCaseId = caseId || data?.case_id || data?.input_case_id || 'CASE-LIVE-CURRENT';
  ...
  if (
    ...
    content = <EvidenceView data={data} onChallengeFinding={setChallengeFinding} />;
  }
  // 2. Contextual Investigation
    ...
    content = <ContextualView data={data} onChallengeFinding={setChallengeFinding} />;
  }
  // 3. Regulatory Risk Assessment
    ...
    content = <RegulatoryView data={data} onChallengeFinding={setChallengeFinding} />;
  }
  // 4. Audit Explanation
  else if (
    normKey === 'audit' || 
    ...
    content = <AuditExplanationView data={data} onChallengeFinding={setChallengeFinding} />;
  }
  // 5. Analyst Decision Support
    ...
    content = <DecisionSupportView data={data} onChallengeFinding={setChallengeFinding} />;
  }
  ...
  return (
    <div className="space-y-4">
      {/* Human Collaboration & Oversight Banner */}
      <div className="p-3 rounded-xl border border-sky-500/25 bg-[#050D1A] flex flex-wrap items-center justify-between gap-2.5 font-mono text-[10px]">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-sky-400 animate-pulse" />
          <span className="font-bold text-sky-300 uppercase tracking-wider">
            HUMAN COLLABORATION PROTOCOL ACTIVE
          </span>
          <span className="text-slate-400 hidden sm:inline">
            · All agent findings are inspectable & challengeable
          </span>
        </div>
        <span className="text-slate-500 uppercase">
          Analyst Decision Authority: <span className="text-emerald-400 font-bold">Unconditional</span>
        </span>
      </div>
      ...
      {/* Finding Challenge / Inquiry Modal */}
      <FindingChallengeModal
        isOpen={!!challengeFinding}
        onClose={() => setChallengeFinding(null)}
