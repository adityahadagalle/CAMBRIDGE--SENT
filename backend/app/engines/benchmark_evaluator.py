...
def evaluate_benchmark_transaction_pure(
    ...
    # 3. Real RandomForest ML Inference
    # Build the 6-feature vector from snapshot fields, matching training schema:
    # [amount, hour, is_new_receiver, velocity, chain_depth, call_flag]
    try:
        from datetime import datetime as _dt_ml
        _ts = snapshot.timestamp
        try:
            _dt = _dt_ml.fromisoformat(_ts.replace("Z", "+00:00"))
            _hour = _dt.hour
        except Exception:
            _hour = 12
    except Exception:
    ...
    feature_vector = [
        float(snapshot.amount),                         # amount (raw, engine normalizes)
        float(_hour),                                   # hour of day
        1.0 if snapshot.is_new_receiver else 0.0,       # is_new_receiver
        5.0 if snapshot.velocity_flag else 1.0,         # velocity (approx from flag)
        float(snapshot.hop_number),                     # chain_depth
        1.0 if snapshot.on_active_call else 0.0,        # call_flag
    ]

    ml_score = predict_ml_score(rule_score, features=feature_vector, seed=snapshot.seed)
